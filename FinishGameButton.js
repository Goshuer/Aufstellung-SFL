...
import { saveData } from "./api"; // vorher gezeigte api.js mit saveData()
...
function FinishGameButton({ players, onSaveComplete }) {
  ...
  const handleFinish = async () => {
    ...
    setError("");
    try {
      // Hier kannst du ggf. Spielwerte aus laufendem Spiel zu dauerhaften Statistiken addieren
      const updatedData = players.map((player) => ({
        ...
        // Lustige Idee: Für nächsten Spielstand Tore/Karten wieder auf 0 setzen hier
        goals: 0,
        yellowCards: 0,
        redCards: 0,
      }));
