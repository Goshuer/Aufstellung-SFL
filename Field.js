...
function Field({ players, onPositionChange, teamColors }) {
  // Spielerpositionen als State - jeder Spieler hat x,y Koordinaten
  ...
  useEffect(() => {
    // Initial Positionen aktualisieren, falls sich Spieler ändern
    setPositions(
      players.reduce((acc, p) => {
  ...
  const onDragEnd = (e) => {
    ...
    // Spielfeld-Position im Verhältnis zum Spielfeld berechnen
    const fieldRect = fieldRef.current.getBoundingClientRect();

    let newX = e.clientX - fieldRect.left - dragOffset.current.x;
    let newY = e.clientY - fieldRect.top - dragOffset.current.y;
    // Spielfeldbegrenzung, falls gewünscht (z.B. min 0, max Spielfeld-Größe)
